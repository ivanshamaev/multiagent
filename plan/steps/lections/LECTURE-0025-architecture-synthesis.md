...
Текст: `course/lectures/LECTURE-0025-architecture-synthesis.md`.
...
- [x] Собрать существующие concepts в четыре слоя и один Net Revenue architecture walkthrough.
- [x] Проследить known facts → requirements → implementation → independent quality → terminal outcome.
- [x] Отдельно показать доказанное offline/live, NEEDS_USER и оставшиеся integration gaps.
- [x] Сформулировать последствия смены одного layer, не повторяя определения/сравнение из 24.
