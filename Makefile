...
.PHONY: secure-env bootstrap lint format-check test plan-check check compose-validate \
	...
	checkpoint-smoke role-pipeline-test data-engineer-live analyst-live requirements-live qa-live reviewer-live quality-loop-live
...
requirements-live: mcp-images
	$(MAKE) --no-print-directory scenario-reset SCENARIO="$(SCENARIO)"
	$(MAKE) --no-print-directory mcp-users
	$(UV) run python -m runtime.requirements_live --scenario "$(SCENARIO)" \
		--analyst-model "$(ANALYST_MODEL)" --pm-model "$(PM_MODEL)" --case "$(ANALYST_CASE)"

qa-live: mcp-images
	$(MAKE) --no-print-directory mcp-users
	$(UV) run python -m runtime.qa_live --mutation "$(QA_MUTATION)" $(if $(QA_MODEL),--model "$(QA_MODEL)")

reviewer-live: mcp-images
	$(MAKE) --no-print-directory mcp-users
	$(UV) run python -m runtime.reviewer_live --mutation "$(REVIEW_MUTATION)" $(if $(REVIEW_MODEL),--model "$(REVIEW_MODEL)")
